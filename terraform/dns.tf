...
resource "azuread_service_principal" "aks_dns_sp" {
  application_id = azuread_application.aks_dns_sp[0].application_id
}
...
resource "azuread_service_principal_password" "aks_dns_sp" {
  service_principal_id = azuread_service_principal.aks_dns_sp[0].id
  value                = random_string.aks_dns_sp[0].result
